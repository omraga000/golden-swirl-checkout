...
export function Header() {
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-40 border-b border-gold/20 backdrop-blur-xl bg-obsidian/70">
      <div className="max-w-7xl mx-auto px-6 h-28 grid grid-cols-[auto_1fr_auto] items-center gap-6">
        ...
        >
          <span
            aria-hidden
            className="absolute inset-0 -m-3 rounded-full bg-[radial-gradient(circle,_rgba(212,175,55,0.45),_transparent_65%)] blur-2xl opacity-70 group-hover:opacity-100 transition-opacity duration-500"
          />
          <span
            aria-hidden
          ...
          <img
            ...
            loading="eager"
            decoding="sync"
            className="relative h-24 md:h-28 w-auto object-contain transition-transform duration-500 group-hover:scale-[1.06] drop-shadow-[0_6px_18px_rgba(212,175,55,0.45)]"
            style={{ imageRendering: "auto" }}
          />
        </a>

        {/* Centered nav with dropdowns */}
        <nav className="hidden md:flex items-center justify-center gap-10 text-[11px] tracking-[0.35em] text-foreground/85">
          {CATEGORIES.map((cat) => (
            <div
              key={cat.key}
              className="relative"
        ...
        <div className="flex items-center justify-end gap-4 shrink-0">
          <div className="relative">
            ...
            {langOpen && (
              <div className="absolute right-0 mt-2 w-32 border border-gold/30 bg-obsidian/95 backdrop-blur-lg animate-fade-up">
